  ...
  return (
    <div className={cn("bg-white rounded-xl border shadow-sm overflow-hidden", className)}>
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b">
        <h3 className="text-base font-semibold">Recent Violations</h3>
        ...
        <table className="w-full">
          <thead className="bg-slate-50/50">
            <tr className="text-left">
              {["S/N", "OFFENDER NAME", "PCN", "OFFENSE TYPE", "PLATE NO.", "AMOUNT", "VIOLATION DATE", "STATUS"].map((header) => (
                <th
                  key={header}
            ...
            {paginatedViolations.map((violation) => (
              <tr key={violation.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-4 py-4">
                  <span className="text-sm font-medium">{violation.offender.name}</span>
                </td>
                <td className="px-4 py-4">
                  <span className="text-sm text-muted-foreground">{violation.pcn}</span>
                </td>
                <td className="px-4 py-4">
                  <span className="text-sm">{violation.offenseType}</span>
                </td>
                <td className="px-4 py-4">
                  <span className="text-sm font-medium">{violation.plateNo}</span>
                </td>
                <td className="px-4 py-4">
                  <span className="text-sm font-semibold">₦{formatCurrency(violation.amount)}</span>
                </td>
                <td className="px-4 py-4">
                  <div className="flex flex-col">
                    <span className="text-sm">{formatDate(violation.violationDate)}</span>
                    <span className="text-xs text-muted-foreground">{formatTime(violation.violationDate)}</span>
                  </div>
                </td>
                <td className="px-4 py-4">
                  <Badge
            ...
            <div key={i}>
              {page === "..." ? (
                ...
                <Button
                  ...
                  className={cn(
                    "h-8 w-8 text-xs",
                    currentPage === page && "bg-slate-900 hover:bg-slate-800"
                  )}
